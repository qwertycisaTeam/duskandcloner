...
function Module:Init(Library, Window, Tab)
    ...
    -- CONFIG MANAGER
    -- ==========================================
    Tab:CreateSection({ Name = "Config System" })

    local ConfigNameInput = Tab:CreateInput({
        Name = "Config Name",
        Placeholder = "Enter config name...",
        Flag = "ConfigManager_Name"
    })
    ...
    local ConfigDropdown = Tab:CreateDropdown({
        ...
        Default = "Select...",
        Flag = "ConfigManager_Selected"
    })

    Tab:CreateButton({
        Name = "Save Config",
        Callback = function()
            local cfgName = Library.Flags["ConfigManager_Name"]
            if cfgName and cfgName ~= "" then
                Library:SaveConfig(cfgName)
                ConfigDropdown:Refresh(Library:GetConfigs())
            else
                if Library.Notify then Library:Notify("Error", "Please enter a config name!", 3) end
            end
        end
    })
    ...
    Tab:CreateButton({
        Name = "Load Config",
        Callback = function()
            local cfgName = Library.Flags["ConfigManager_Selected"]
            if cfgName and cfgName ~= "Select..." then
                Library:LoadConfig(cfgName)
            end
        end
    })

    Tab:CreateDivider({ Text = "Auto-Load" })

    Tab:CreateToggle({
        Name = "Auto-Load Settings",
        Description = "Automatically loads 'autoload.json' when injected.",
        Flag = "Settings_AutoLoad",
        Default = false,
        Callback = function(state)
            if state then
                Library:SaveConfig(Library.AutoLoadFile)
            else
                Library:DeleteConfig(Library.AutoLoadFile)
            end
        end
    })
    -- ==========================================
