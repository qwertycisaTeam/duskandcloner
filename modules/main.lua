...
function Module:Init(Library, Window, Tab)
    ...
    -- Адаптивный заголовок (цвет привязан к теме)
    ...
    -- ЭЛЕГАНТНАЯ РАЗДЕЛИТЕЛЬНАЯ ЛИНИЯ ПОД ШАПКОЙ (Адаптивная)
...
function Module:CreateFileCard(fileName)
    ...
    RenameBox:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        -- Вырезаем запрещенные символы ОС + все спецсимволы (оставляем буквы, цифры, пробелы)
